 ...
 * write in the same place in the teardown with none of the timing, so what is
 * under test is the ordering and not the scheduler.
 *
 * It renders a marker, so a case that runs after the teardown can tell an
 * unmount that happened from one that never did.
 */
...
// These three cases are one assertion split across a teardown, so their order
// matters. A hook that runs between tests cannot be observed from inside one.
describe("test setup teardown", () => {
  // Without this, the pair below passes whether the fixture writes anything at
  // all, and the guard would go dead in silence the day the fixture stopped
  // writing.
  ...
  it("resets the address after the unmount rather than before it", () => {
    ...
    // because the runner would then run the resets first and this case would
    // open on the previous one's leaked address.
    expect(window.location.search).toBe("");
    // And fails if the unmount stops happening at all, which would otherwise
    // leave this case green because nothing tore down and so nothing leaked.
