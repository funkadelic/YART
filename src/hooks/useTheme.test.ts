 ...
 * constructed in this file, because the hook schedules nothing, so a failure
 * here points at the hook and not at the runner's timer handling.
 */
function renderTheme() {
  return renderHook(() => useTheme());
}

/** What the document element is actually carrying. */
function stampedTheme(): string | null {
  return document.documentElement.getAttribute("data-theme");
}

/**
 * The inline color scheme, the half of the stamp that decides native control
 * and scrollbar appearance.
 */
...
describe("useTheme", () => {
  ...
  describe("the operating system subscription", () => {
    ...
    it("re-reads the preference as it subscribes, so a change during the render is not lost", () => {
      ...
      // and reproducing it needs a preference that moves on its own, because a
      // test can only move one between commits.
