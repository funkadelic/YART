 ...
 *
 * What this proves that no jsdom assertion can: two documents own two
 * independent addresses. jsdom has one document, so the claim that a view
 * restored on the films page leaves the city page's address alone cannot be
 * made there at all, and the claim that a reload reproduces a view is made
 * against a remount that keeps the module graph and the parsed dataset alive
 * across the boundary it calls a reload.
 *
...
// indexes it, and only then waits out the seam's deliberate latency. Raised
// deliberately rather than as a flake allowance.
...
// expectation is computed through the platform rather than typed: a separator
// written by hand is the one thing a failure here cannot show.
...
// the sentinel needs only to be somewhere Back can land that the application did
// not put there.
...
test("each of the four view changes reaches the films address", async ({
  ...
  // The default view writes nothing, so the shell's own address is what the
  // three changes below are measured from.
  await expect(page).toHaveURL(FILMS_PATH);

  // Waited on the address rather than slept on, because the write is an effect
  // and each interaction returns before it commits. The search commit waits out
  // the debounce on top of that.
...
test("a redundant parameter is dropped and an unowned one survives", async ({
  ...
  // rather than reaching the table. This is one view having exactly one address,
  // asserted from the arrival side.
...
test("paging the films page adds no history entry", async ({ page }) => {
  ...
  // The criterion, made falsifiable by the sentinel: if this page ever pushed an
  // entry, Back would land on an intermediate films address and this goes red.
  await page.goBack();
  await expect(page).toHaveURL(SENTINEL_PATH);

  // Forward restores the entry's current address, and the entry's current
  // address is the one the in-place write left behind, which is what proves the
  // write mutated the entry rather than adding to it.
...
test("the two pages own independent addresses", async ({ page }) => {
  ...
  // Back to the first page, which is a different document. Its own view is
  // intact, which is the half one document cannot demonstrate.
  await page.goBack();
  await expect(page.getByRole("table")).toBeVisible({
    timeout: DATASET_READY_TIMEOUT_MS,
  });
...
test("a films search matching nothing empties the results and no more", async ({
  ...
  // documented branch order. What must not happen is the whole view going back
  // to the download sentence: that branch is gated on the dataset having
  // arrived once, so a refetch marks the view busy rather than tearing it down.
