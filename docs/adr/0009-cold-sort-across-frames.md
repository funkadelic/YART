...
Each column and direction a reader sorts keeps two arrays of row references alive, the rows it was given and their order, for as long as its column array exists. Measured in Chromium, a 50,250-row array of references is about 200 KB, so the ten city orders come to about 2 MB. Every unfiltered fetch returns a fresh copy of the rows, so the stored input is often an array the app has already replaced. In the worst case those inputs keep about another 2 MB alive. The first sort of each column and direction still compares every row. Measured in Chromium at fourfold slowdown over evenly spaced subsets of both datasets, 5,000 rows take a median 41 ms to sort by film genres, the costliest comparator, 23 ms by city name and 18 ms by film title. Genres crosses the 50 ms long-task line at 6,000 rows, city name at about 9,000. Browsers without `scheduler.yield` pay the 4 ms minimum delay on nested timers at each yield.
