...
Coverage from several runners can be merged, and merging only ever raises the number. Merging puts the floor on the slowest and most engine-dependent suite in the tree, so a flaky browser run becomes a coverage failure and the way to close a gap becomes "add an end-to-end test."
...
A gap has to be closed by a fast test. The browser suites are free to assert what only a real engine can decide, without their line counts meaning anything.
