...
The suites under `bench/` measure the table engine: the sort over a ten thousand row set through both tables' shipping comparators, the page slice over a set the size of the city asset, the reducer and the address the view state is written to, the column arrays and the cells they render, and the locale layer that builds the labels and weaves the sentences a render shows. Every one of them calls the shipping module, so a benchmark cannot pass while measuring a copy of the code that drifted from it.

The rows are generated by a seeded generator in `bench/fixtures.ts` rather than read off the committed datasets, which reach the app as URL assets a benchmark process has no bundler to resolve. The same seed produces the same rows on every machine, so two runs differ only where the code does.

```bash
npm run bench
```

That prints a table per suite locally. In the pipeline the same command runs under [CodSpeed](https://app.codspeed.io/funkadelic/YART), which measures the work each benchmark performs by simulating a CPU rather than timing it, so a hosted runner's noise does not reach the number and a pull request is reported against its base. A single suite can be run on its own with `npx tsx bench/sortRows.bench.ts`.

The benchmarks are not a gate: they run in a workflow of their own, and a regression is reported on the pull request rather than failing the merge.
