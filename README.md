...
The committed asset is `src/data/worldcities/cities.json`, 50,250 rows.
`scripts/generate-cities.mjs` serialized it from the row data this repository
already carried, not from a fresh run over the upstream CSV export. Later
revisions come from that script reading the upstream `worldcities.csv`, which
orders rows by descending population and breaks ties by ascending id. Expect a
regenerated file to differ from the committed one.
`src/data/worldcities/license.txt` records the same provenance.
